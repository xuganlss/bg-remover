// Pricing page with subscription management and credit ledger
'use client';
    ...
    >
      <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 text-sm text-gray-600">
            ...
            <Link href="/pricing" className="hover:text-purple-600 transition-colors">Credit Packs</Link>
            {user ? (
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-extrabold text-gray-900 mb-3">Subscription & Credit Management</h1>
          <p className="text-gray-500 text-lg">Manage your subscription, track credit usage, and view transaction history</p>
        </div>
        ...
        {/* User Account Section - Only shown when logged in */}
        {user && accessToken && (
          <div className="mb-12">
            <div className="bg-gradient-to-br from-purple-600 to-blue-600 rounded-2xl p-6 text-white mb-8">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-bold">👋 Welcome, {user.name.split(' ')[0]}!</h2>
                <div className="flex items-center gap-2">
                  <span className="text-sm bg-white/20 px-3 py-1 rounded-full">{credits} credits</span>
                </div>
              </div>
              <p className="text-purple-100 text-sm">
                Manage your subscription below or view your complete credit history.
              </p>
            </div>
        ...
        {/* Features Comparison */}
        <div className="mb-16">
          <h2 className="text-2xl font-bold text-gray-800 text-center mb-8">Plan Comparison</h2>
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className="grid grid-cols-4 border-b border-gray-200 bg-gray-50">
              <div className="p-4 font-semibold text-gray-700">Feature</div>
              <div className="p-4 font-semibold text-gray-700 text-center">Credit Packs</div>
              <div className="p-4 font-semibold text-gray-700 text-center bg-purple-50">Basic</div>
              <div className="p-4 font-semibold text-gray-700 text-center bg-purple-50">Pro</div>
            </div>
            {[
              ['Price', 'Pay as you go', '$9.99/month', '$19.99/month'],
              ['Credits', 'Never expire', '25/month', '60/month'],
              ['Cost per image', 'From $0.37', '$0.40', '$0.33'],
              ['Rollover credits', '✅ Yes', '✅ Yes', '✅ Yes'],
              ['HD output', '✅ Yes', '✅ Yes', '✅ Yes'],
              ['Support', 'Email', 'Email', 'Priority'],
              ['API access', '❌ No', '❌ No', '✅ Yes'],
              ['Cancel anytime', 'N/A', '✅ Yes', '✅ Yes'],
            ].map(([feature, pack, basic, pro], i) => (
              <div key={i} className={`grid grid-cols-4 border-b border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                <div className="p-4 font-medium text-gray-900">{feature}</div>
                <div className="p-4 text-gray-600 text-center">{pack}</div>
                <div className="p-4 text-gray-600 text-center bg-purple-50/50">{basic}</div>
                <div className="p-4 text-gray-600 text-center bg-purple-50/50">{pro}</div>
              </div>
            ))}
          </div>
        </div>
